...
#define LOAD_FACTOR(N,M) (((double)(N) / (M) >= 1.5) ? 1:0)                             //macro na vypocet pomeru medzi velkostou tabuľky a počtom pridaný prvkov
#define HASH_FUNCTION(x,M) ((x)%(M))                                                    //hešovacia funkcia
#define MIN_SIZE 13                                                                     //mnou zvolena minimalna velkost hešovacej tabuľky
...
int insert_HASH(int number, TABLE **table){
    ...
    (*table)->elements++;
    if(LOAD_FACTOR((*table)->elements,(*table)->size)){                          //ak je LOAD_FACTOR väčší ako mnou zvolený pomer (1,5)
        resize(table);                                                                  //tak vykonám zväčšenie tabuľky
    }
    return 1;
}
